...
export default function MicroclimateTelemetryCard({ data, loading, onBroadcastClick }: MicroclimateTelemetryCardProps) {
  ...
  // Accurate OSHA strain index
  const isExtreme = ambientTemp >= 104 || surfaceTemp >= 118
  const isElevated = (ambientTemp >= 95 || surfaceTemp >= 106) && !isExtreme

  const strainText = isExtreme ? 'Severe Thermal Hazard (15/45 Work/Rest)' : isElevated ? 'Elevated Strain (50/10 Ratio)' : 'Normal Work Parameters'
  ...
  return (
    <div className="card-warm p-4 space-y-3.5 font-mono border-[#A27B5C]/40 shadow-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-[#A27B5C] font-semibold">
              Hyperlocal 100m Physics Simulation
            </span>
          </div>
        </div>
      ...
      {/* 3-Way Thermal Real-Time Metrics */}
      <div className="grid grid-cols-3 gap-2 text-center">
        ...
        <div className="p-2.5 rounded-xl bg-[#1A2224] border border-emerald-500/30 space-y-1">
          <span className="text-[9px] text-[#DCD7C9]/60 uppercase block">Shaded Canopy</span>
      ...
      {/* OSHA WBGT Thermal Strain Status */}
      <div className={`p-2.5 rounded-xl border ${strainColor} flex items-center justify-between text-xs`}>
        <div className="flex items-center gap-2">
          <span>{isExtreme ? '🚨' : isElevated ? '⚠️' : '🛡️'}</span>
          <span className="font-bold font-sans text-[11px]">{strainText}</span>
        </div>
        <span className="text-[10px] font-mono opacity-80">OSHA WBGT</span>
      </div>

      {/* Autonomous ThermaShift Directive Banner */}
      <div className="p-3 rounded-xl bg-gradient-to-r from-red-950/40 via-[#242D30] to-emerald-950/40 border border-[#A27B5C]/50 space-y-2">
        <div className="flex items-center justify-between text-[10px]">
          <span className="font-bold text-[#A27B5C] uppercase flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            Autonomous ThermaShift Directive
          </span>
          <span className="text-emerald-400 font-bold">
            -{reliefDelta}°F Shift Benefit
          </span>
        </div>
        ...
        <div className="flex items-center justify-between text-[10px] pt-1.5 text-[#DCD7C9]/70 border-t border-[#3F4E4F]/40 font-sans">
          <span>Strain Reduction: <strong className="text-emerald-300 font-mono">42%</strong></span>
          <button 
