...
export default function ProductStoryModal({ onClose, onTriggerTest }: ProductStoryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-sm animate-fade-in font-sans">
      <div className="bg-[#11171B] text-slate-200 border border-slate-700/80 rounded-2xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        {/* Top Header */}
        ...
        <div className="shrink-0 p-3 bg-[#141B20] border-b border-slate-800/80 grid grid-cols-4 gap-2">
          {STORY_STEPS.map((s, idx) => (
            <button
              ...
              className={`p-2 rounded-xl text-left transition-all border ${
                activeStep === idx
                  ? 'bg-[#182228] border-slate-600 shadow-sm'
                  : 'bg-[#11171B] border-slate-800/80 hover:border-slate-700'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs">
          ...
          <div className="flex items-center gap-2">
            <span
              className="text-[10px] font-semibold tracking-wider uppercase px-2 py-0.5 rounded-md border"
              style={{
          ...
          {/* Body Paragraph */}
          <p className="text-xs text-slate-400 leading-relaxed bg-[#141B20] p-4 rounded-xl border border-slate-800">
            {current.description}
          </p>
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {current.metrics.map((m) => (
              <div key={m.label} className="p-3 rounded-xl bg-[#141B20] border border-slate-800/80 text-center">
                <span className="text-[10px] text-slate-400 block mb-0.5">{m.label}</span>
                <span className="text-xs font-semibold text-slate-100 block">{m.value}</span>
              </div>
            ))}
          </div>
          ...
          >
            <div className="w-5 h-5 rounded-md flex items-center justify-center shrink-0 mt-0.5" style={{ color: current.accent }}>
              ✓
            </div>
            <p className="text-slate-300 text-[11px] leading-relaxed">
              <strong className="text-white">Key Impact: </strong>
              {current.highlight}
            </p>
          </div>
        </div>
